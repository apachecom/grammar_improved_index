...
namespace csd {
    ...
    size_t
    CSD_HTFC::getSize() {
        return bytes * sizeof(uchar) + blocks->getSize() + tree->getSize() + (tree->getLength() / 2) * sizeof(uint) +
               leafs * sizeof(Tcode) + sizeof(CSD_HTFC);
    }
    ...
    CSD_HTFC::locateLimitBlocks(uchar *s, uint slen, int *left, int *right) {
        ...
        for (uint i = 0; i < slen; i++)
            encodeHT(HTcode[s[i]].code, HTcode[s[i]].cbits, encoded, &encpos, &encoffset);
        if (encoffset > 0) encpos++;
